...
# ╔═╡ ac5f559e-6c20-11ef-174a-5b9f735d0e18
begin
	using Unfold, UnfoldMakie, UnfoldSim
	using DataFrames, DataFramesMeta
	using PlutoUI
	using Random, CairoMakie
	using ArrayPadding, Statistics
end

# ╔═╡ 5e502766-5c1a-492f-84f8-11c49ebef31a
begin
...
# ╔═╡ b9104e44-0209-4c6b-9d6f-05c99ea69efc
md"""
# Calculate MSE
"""
...
# ╔═╡ bdfa73b9-0c9d-4730-aafc-7094649cff2d
begin
	components = [p1, n1, p3]
	data, evts = simulate(
	    MersenneTwister(1),
	    design,
	    components,
	    UniformOnset(; width = 30, offset = 5),
	    PinkNoise(;noiselevel = nl),
	);
end

# ╔═╡ 09942c30-fca9-4222-8426-ac83d1fab930
m = fit(
    UnfoldModel,
...
# ╔═╡ 38f1771b-7e63-44b0-b4bc-253f96485a18
plot_erp(
    coeftable(m);
    axis = (
...
begin
	s = size(@rsubset(ef, :condition == "car", :continuous == 0).yhat, 1)
	res = zeros(length(unique(ef.continuous)), s);
	for (i, c) in enumerate(unique(ef.continuous))
		tmp = @rsubset(ef, :condition == "car", :continuous == c).yhat
		res[i, :] = tmp
	end
	res'
...
# ╔═╡ 113932da-9bc8-4473-bbec-20ef7640cb89
lines(data[500:1000])

# ╔═╡ b448cdb5-4ca6-4449-b730-df6eada6e100
size(evts)

# ╔═╡ d6b7be91-6e6b-4043-8c41-371f26db7071
let
	p3t = LinearModelComponent(;
    basis = p300(),
    formula = @formula(0 ~ 1 + condition + continuous^2 + cont2),
    β = [5, 1, 0.2, 0.1],
	);
	
	components_t = [p1, n1, p3t]

	simt = UnfoldSim.Simulation(
		SingleSubjectDesign(;
        conditions = Dict(
            :condition => ["car", "face"],
	...
	gtt = UnfoldSim.simulate_responses(MersenneTwister(1),components_t, simt)
end

# ╔═╡ d0d1b6c7-0694-48e3-8563-2b3cc7ccd3ba
begin
	# Plot simulated ERP with effect
	#lines(components[1].basis + components[2].basis)
...
# ╔═╡ 7387483e-95be-426f-be94-e2c8f7488a07
begin
	τ = [-0.1, 1]
	sfreq = 100
	if τ[1] < 0
		st = Int64(abs(τ[1] * sfreq))
	else
		st = 0
	end
	et = Int64((τ[2] * sfreq) - size(gt, 1)) + 1
	pad(gt, 0, (st, 0), (et, 0))

end

# ╔═╡ cc4d39cc-47d3-401b-acd6-98f37cc1b601
τ[1] * sfreq

# ╔═╡ e4f43226-fa01-4e7f-ac2e-6d3ddbd63ef3
begin
	slength = 10
	elength = 101 - size(gt, 1)
	gt_pad = pad(gt, 0, (slength, 0), (elength, 0))
end

# ╔═╡ 0923d2e1-bf52-4d53-9ef4-5aab34b5b172
let	ft = Figure()
	axt = Axis(ft[1,1])
	for (i,p) in enumerate(eachcol(gt_pad))
		lines!(axt, p, color= :blue)
		lines!(axt, res[i,:], color= :red)
	end
	ft
end
...
manifest_format = "2.0"
project_hash = "cd0e3adf299e24a4c2c185042d451000b3fb53dd"
...
deps = ["Colors", "Dates", "Dictionaries", "FileIO", "GLM", "GeoInterface", "GeometryBasics", "GridLayoutBase", "Isoband", "KernelDensity", "Loess", "Makie", "NaturalSort", "PlotUtils", "PolygonOps", "PooledArrays", "PrecompileTools", "RelocatableFolders", "StatsBase", "StructArrays", "Tables"]
git-tree-sha1 = "dca7c14d669dfb83cba7f4cc670fda9b6721fef2"
...
[[deps.ArrayPadding]]
deps = ["Functors", "GPUArraysCore", "Lazy", "Statistics"]
git-tree-sha1 = "4e70ea8f6463e8ef108b451cd7ab454a732f894c"
...
git-tree-sha1 = "783507c1f2371c8f2d321f41c3057ecd42cafa83"
...
git-tree-sha1 = "55b9364f382b37c0794d7e8065aefceaa225c04c"
...
git-tree-sha1 = "492d8041f5ef9dffad4b91b6cd5075adf70cc4d2"
...
git-tree-sha1 = "1cf5ee2bfc0db76d9aca62bf4b16088e6722c055"
...
git-tree-sha1 = "d2523bfddde4112695ed01020e722418660620aa"
